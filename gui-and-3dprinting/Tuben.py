...
from new_main_window import Ui_TubeN
import formantsynt
...
class AppWindow(QMainWindow, Ui_TubeN):
    ...
    def visualization(self, l, a):
        ...
        margin_factor = 1.8  # 越大管子占窗口越小
        rect_with_margin = rect.adjusted(
            ...
            if isinstance(event, QKeyEvent):
                ...
                if self.index is not None:
                    ...
                    if event.key() == Qt.Key_Up:
                        self.A[self.index] += step
                    elif event.key() == Qt.Key_Down:
                        self.A[self.index] = max(0.1, self.A[self.index] - step)
                    elif event.key() == Qt.Key_Right:
                        self.L[self.index] += step
                    elif event.key() == Qt.Key_Left:
                        self.L[self.index] = max(0.1, self.L[self.index] - step)
